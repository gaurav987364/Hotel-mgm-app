...
// const SalesChart = ({booking, numDays}) => {
...
//       totalSales : booking.filter(booking => isSameDay(date, new Date(booking.created_at))).reduce((acc , cur)=> acc+cur.totalPrice, 0),
//       extrasSales : booking.filter(booking => isSameDay(date, new Date(booking.created_at))).reduce((acc , cur)=> acc+cur.extrasPrice, 0),
//     }
// });
// console.log(data);
...
const SalesChart = ({ booking = [], numDays = 30 }) => {
  const { isDarkMode } = useDarkMode();
    ...
    <StyledSalesChart>
      <Heading as="h2">
        ...
        <strong style={{ color: "purple" }}>
          {formatDate(allDates.at(0), "MM dd yyyy")}
        </strong>{" "}
        &mdash;{" "}
        <strong style={{ color: "purple" }}>
...
SalesChart.propTypes = {
  booking: PropTypes.arrayOf(PropTypes.object),
